...
async def get_my_cheer_messages(authorization: str = Header(None), race_code: str = None):
    ...
    for rc, bibs in bib_map.items():
        ...
        for msg in messages:
            all_messages.append({
                # El identificador es lo que permite responder desde el perfil
                "id": str(msg["_id"]),
                "athlete_bib": msg.get("athlete_bib"),
                "fan_name": msg.get("fan_name", "Anonimo"),
                "message": msg.get("message", ""),
                "race_code": rc,
                "race_name": race_name,
                "created_at": msg.get("created_at").isoformat() if msg.get("created_at") else None,
                "reply": msg.get("athlete_reply"),
                "replied_at": msg.get("athlete_reply_at").isoformat() if msg.get("athlete_reply_at") else None,
            })
...
async def _dorsales_del_atleta(database, athlete_id: str, athlete: dict) -> set:
    """Todas las formas del dorsal con las que puede aparecer este corredor.

    Sirve para comprobar que un mensaje va dirigido a el antes de dejarle
    responderlo: sin esto, con el identificador de otro mensaje se podria
    contestar en nombre de cualquiera.
    """
    from bson import ObjectId
    ...
    registros = await database.registrations.find(
        {"$or": [{"athlete_id": athlete_id}, {"email": athlete.get("email")}]},
        {"bib": 1},
    ).to_list(100)
    for reg in registros:
        if reg.get("bib"):
            dorsales.add(reg["bib"])

    # Resultados de anos anteriores que el corredor reclamo como suyos
    reclamados = athlete.get("claimed_results") or []
    if reclamados:
        try:
            ids = [ObjectId(c) for c in reclamados]
            for coleccion in ("participants", "archived_participants"):
                async for doc in database[coleccion].find({"_id": {"$in": ids}}, {"bib": 1}):
                    if doc.get("bib"):
                        dorsales.add(doc["bib"])
        except Exception:
            pass

    # El dorsal se guarda como "042", "42" o 42 segun de donde venga
    formas = set()
    for b in dorsales:
        texto = str(b)
        formas.add(texto)
        sin_ceros = texto.lstrip("0") or "0"
        formas.add(sin_ceros)
        formas.add(sin_ceros.zfill(3))
        if sin_ceros.isdigit():
            formas.add(int(sin_ceros))
    return formas


@router.post("/my-messages/{cheer_id}/reply")
async def responder_mensaje_de_animo(
    cheer_id: str,
    datos: RespuestaMensaje,
    authorization: str = Header(None),
):
    """El corredor contesta un mensaje de apoyo que le escribieron.

    La respuesta se guarda junto al mensaje y sale tambien en el muro publico:
    quien se molesto en escribir merece ver que le contestaron, y ahi es donde
    lo va a buscar.
    """
    from server import db as database
    from bson import ObjectId
    ...
    respuesta = (datos.reply or "").strip()
    if len(respuesta) > MAX_RESPUESTA:
        raise HTTPException(
            status_code=400,
            detail=f"La respuesta no puede exceder {MAX_RESPUESTA} caracteres",
        )
    ...
    for coleccion in (database.cheer_messages, database.archived_cheer_messages):
        ...
        if respuesta:
            cambio = {"$set": {
                "athlete_reply": respuesta,
                "athlete_reply_at": datetime.now(timezone.utc),
            }}
        else:
            # Respuesta vacia: se borra, que es como se desdice uno
