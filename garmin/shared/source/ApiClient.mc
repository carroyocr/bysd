...
class ApiClient {
    ...
    // La firma la fija makeWebRequest y no se puede aflojar: el comprobador de
    // tipos exige exactamente estos tipos y 'as Void'. El servidor puede
    // responder texto en vez de JSON si algo va mal por el camino, y por eso
    // 'datos' no es solo un diccionario.
    function onLapStatus(codigo as Lang.Number,
                         datos as Lang.Dictionary or Lang.String or Null) as Void {
        if (codigo != 200 || !(datos instanceof Lang.Dictionary)) {
            _enVuelo = false;
            return;
        }
    ...
    function onStats(codigo as Lang.Number,
                     datos as Lang.Dictionary or Lang.String or Null) as Void {
        _enVuelo = false;
        if (codigo != 200 || !(datos instanceof Lang.Dictionary)) { return; }
